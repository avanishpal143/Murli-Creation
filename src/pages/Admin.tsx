...
export default function AdminPage() {
  ...
  const handleAddProduct = (e: React.FormEvent) => {
    ...
    const imagesList = imageUrl.split(",").map(url => url.trim()).filter(url => url.length > 0);
    const finalImages = imagesList.length > 0 ? imagesList : ["/images/products/lippan_tree.jpg"];
    ...
    setFolklore("");
    setImageUrl("/images/products/lippan_tree.jpg");
    setVideoUrl("");
    setDescription("");
    setIsAdding(false);
  };
            ...
            {isAdding && (
              <div className={styles.formSection}>
                ...
                <form onSubmit={handleAddProduct} className={styles.formGrid}>
                  ...
                  <div className={styles.inputGroup}>
                    <label className={styles.label}>Image URLs (Comma separated for multiple)</label>
                    <input type="text" value={imageUrl} onChange={(e) => setImageUrl(e.target.value)} className={styles.input} placeholder="/images/products/lippan_tree.jpg, /images/products/ig_post_1.png" />
                  </div>
                  <div className={styles.inputGroup}>
                    <label className={styles.label}>Video URL (Optional)</label>
                    <input type="text" value={videoUrl} onChange={(e) => setVideoUrl(e.target.value)} className={styles.input} placeholder="e.g. /images/products/video1.mp4 or YouTube link" />
                  </div>
            ...
            {editingProduct && (
              <div className={styles.formSection}>
                ...
                <form onSubmit={handleUpdateProductSubmit} className={styles.formGrid}>
                  ...
                  <div className={styles.inputGroup}>
                    <label className={styles.label}>Image Paths (Comma separated for multiple)</label>
                    <input
                      type="text"
                      value={editingProduct.images.join(", ")}
                      onChange={(e) => setEditingProduct({ ...editingProduct, images: e.target.value.split(",").map(u => u.trim()).filter(u => u.length > 0) })}
                      className={styles.input}
                    />
                  </div>
                  <div className={styles.inputGroup}>
